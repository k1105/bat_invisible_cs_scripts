...
public class OVRCustomPlayerController : MonoBehaviour
{
	...
	protected virtual void UpdateController()
	{
		...
		MoveThrottle.y /= motorDamp; //= (MoveThrottle.y > 0.0f) ? (MoveThrottle.y / motorDamp;
	...
	public virtual void UpdateMovement()
	{
		...
		if (EnableLinearMovement)
		{
			...
			if (moveForward)
				MoveThrottle += ort * (transform.lossyScale.z * moveInfluence * forward);
			if (moveBack)
	...
	public void UpdateTransform(OVRCameraRig rig)
	{
		...
		if (HmdRotatesY && !Teleported)
		{
			...
			transform.rotation = Quaternion.Euler(centerEye.rotation.eulerAngles);
